...
/* This does't work ATM. IonWS split table will be corrupt when it
 * tries to find out the split for a in-process-of-being destroyed
 * frame. --> stick to weak tables.
 */
...
static void extl_push_obj(lua_State *st, WObj *obj)
{
    ...
    init_watch(watch);
    setup_watch(watch, obj, NULL /*extl_obj_dest_handler*/);
...
static bool extl_init_obj_info(lua_State *st)
{
    ...
    /* Create proxy cache and mark it weak */
    lua_newtable(st);
    lua_newtable(st);
